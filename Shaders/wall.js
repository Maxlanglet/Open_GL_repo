var load_shader_wall = async function(gl) {
    const shader_V = `
    ...
    const wall_texture =  make_texture(gl,"../Objects/Room-SW/textures/Concreto_color.png");
    const light_pos = glMatrix.vec3.fromValues(1.0, 0.0, -1.8);

    function shader_activate(shader,model,pos_model){
        //itM matrix
        u_itM = gl.getUniformLocation(shader.program,"itM")
        var itM = glMatrix.mat4.create();
        itM = glMatrix.mat4.invert(itM, model);
        itM = glMatrix.mat4.transpose(itM, itM);
        gl.uniformMatrix4fv(u_itM, false, itM);
        //Model
        gl.uniformMatrix4fv(pos_model, false, model);
